...
clasMatDetec = claseControladorDetectorDeMatriculas(threshold = 0.3,
                                                    batch_size = 30,
...
class matDetec():
    ...
    def __init__(self):
        ...
        if os.path.exists(clasMatDetec.h5):
            ...
            print('')
            self.model = tf.keras.models.load_model(clasMatDetec.h5, custom_objects={'lossFunction': self.loss_function})
...
matDetecClass = matDetec()

if False:

    cap = cv2.VideoCapture("videoCochesBarcelona.mp4")
    ...
    while True:
        if (cap.isOpened()== False):
            ...
            """
            if contador%20 == 0:
                cv2.imwrite("algunosFramesParaEtiquetar/videoBarcelona" +str(contador).zfill(10) + ".jpg", frame)
            ...
            imgOut,_= matDetecClass.matDetecFunction(frame)

            cv2.imshow('frame',cv2.resize(imgOut, (1280,620)))
...
if False:
    ...
    def filesRead(_path, archivosPermitidos):
        filesNomb = []
    ...
    filesNomb = filesRead("baseDeDatos/", ["jpg", "png", "JPG", "jpeg"])
    shuffle(filesNomb)

    for name, contador in zip(filesNomb, range(len(filesNomb))):

        img = cv2.imread(name)
        imgOut,boxes = matDetecClass.matDetecFunction(img)
        
        cv2.imshow("img", cv2.resize(imgOut, (800, 600)))
        cv2.waitKey(0)
