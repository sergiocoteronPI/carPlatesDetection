...
try:
    ...
    input("Error: no se ha podido realizar: font = cv2.FONT_HERSHEY_SIMPLEX")
    ...
    if im_sha_1 >= self.dim_fil:
        if im_sha_2 >= self.dim_col:
            try:
                ...
                print("Error al ejecutar: zeros = cv2.resize(img,(self.dim_col,self.dim_fil))")
            for obj in cosico:
            ...
            try:
                ...
                print("Error al ejecutar: zeros[:,0:im_sha_2,:] = cv2.resize(img,(im_sha_2,self.dim_fil))")
            for obj in cosico:
        ...
        try:
            ...
            print("Error al ejecutar: zeros[0:im_sha_1,:,:] = cv2.resize(img,(self.dim_col,im_sha_1))")
        for obj in cosico:
...
def dibujar_imagen_y_coordenadas(classMatDetec, image, final_bbox):
    ...
    for box in final_bbox:
        ...
        try:
            ...
            cv2.rectangle(image,(left, top), (right, bot),classMatDetec.colors[classMatDetec.labels.index(mess)], 2)
                
            if top - 16 > 0:
                cv2.rectangle(image,(left-1, top - 16), (left + (len(mess))*5*2-1, top),classMatDetec.colors[classMatDetec.labels.index(mess)], -1)
                cv2.putText(image,mess ,(left, top - 4), font, 0.45,(0,0,0),0,cv2.LINE_AA)

            else:
                cv2.rectangle(image,(left-1, top), (left + (len(mess))*5*2-1, top+16),classMatDetec.colors[classMatDetec.labels.index(mess)], -1)
                cv2.putText(image,mess,(left, top + 12), font, 0.45,(0,0,0),0,cv2.LINE_AA)
        except:
            print("Error en dibujar_imagen_y_coordenadas")

def visualiza(classMatDetec):
#if True:
    
    image_label_nomb = leerDatosTxt(ruta = classMatDetec.rpe)
    ...
    for name in image_label_nomb:
        ...
        try:
            image = cv2.imread(classMatDetec.rpi + vector[0][0])
        except:
            print("Error al abrir: ", name)
            print("Error en visualiza al tratar de ejecutar image = cv2.imread(classMatDetec.rpi + vector[0][0])")
        ...
        try:
            ...
            print("Error en visualiza")

#Cosa para visualizar
#visualiza(classMatDetec)
...
def agrandar_bboxes(classMatDetec, image, bboxes):
...
def cargarLote(self, nom_lot):
    ...
    for name in nom_lot:
        ...
        if len(vector[0]) == 1:
            ...
            transformed_image, _ = retocar(self, image,[])
...
if False:
    ...
    for name in image_label_nomb:
        ...
        image = leer_imagen_en_eg_o_color(classMatDetec.rpi + vector[0][0])
        image_original, _ = retocar(classMatDetec, deepcopy(image), [])
        ...
        image, bboxes = retocar_imagen_y_coordenadas(image, bboxes)
        image, bboxes = retocar(classMatDetec, image,bboxes)
...
if False:
    ...
    for name in image_label_nomb:
        ...
        with open(name, 'r') as f:
            for line in f:
                linea = line.rstrip('\n').split(',')
                if linea[1] in classMatDetec.labels:
                    vector.append(linea)
        ...
        for box in bboxes:
            ...
            for i,j in [(0,1),(1,1),(1,0), (0,-1),(-1,-1),(-1,0), (1,-1), (-1,1)]:
                ...
                if (cx + i) > 0 and (cx + i) < W and (cy + j) > 0 and (cy + j) < H:
                    ...
                    cv2.rectangle(image,(new_left, new_top),(new_right, new_bot),classMatDetec.colors[classMatDetec.labels.index(mess)], 1)
